<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between">
            ...
            <form action="{{ route('referenciels') }}" method="GET">
        ...
        <div>
            <a class="p-2 bg-gray-500 hover:bg-gray-700 text-white font-bold rounded" href="{{ route('referenciels/form') }}">{{ __('other.add_referenciel') }}</a>
        </div>
    </x-slot>
                ...
                <div class="overflow-hidden">
                    <table class="min-w-full mb-3">
                        ...
                        <tr>
                            ...
                            <th scope="col" class="text-sm font-medium text-gray-900 px-6 py-4 text-left">
                                @sortablelink('libelle', "Libellé")
                            </th>
                            <th scope="col" class="text-sm font-medium text-gray-900 px-6 py-4 text-left">
                                @sortablelink('annees', "Années")
                            </th>
                            <th scope="col" class="text-sm font-medium text-gray-900 px-6 py-4 text-left">
                                @sortablelink('niveau', "Niveau")
                            </th>
                        </tr>
                        </thead>
                        <tbody>
                        @foreach ($referenciels as $referenciel)
                            <tr class="bg-gray-100 border-b">
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{$referenciel->id}}</td>
                                <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">{{$referenciel->libelle}}</td>
                                <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">{{$referenciel->annees}}</td>
                                <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">{{$referenciel->niveau}}</td>
                            </tr>
                        @endforeach
                        </tbody>
                    </table>
                    <div>
                        {{ $referenciels->links('pagination::tailwind')}}
                    </div>
                </div>
            </div>
        </div>
