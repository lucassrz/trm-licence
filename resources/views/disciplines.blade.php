<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between">
            ...
            <form action="{{ route('disciplines') }}" method="GET">
        ...
        <div>
            <a class="p-2 bg-gray-500 hover:bg-gray-700 text-white font-bold rounded" href="{{ route('disciplines/form') }}">{{ __('other.add_discipline') }}</a>
        </div>
    </x-slot>
                    ...
                    <table class="min-w-full mb-3">
                        ...
                        @foreach ($disciplines as $discipline)
                            <tr class="bg-gray-100 border-b">
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{$discipline->id}}</td>
                                <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">{{$discipline->code}}</td>
                                <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">{{$discipline->libelle}}</td>
                                <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">{{$discipline->created_at}}</td>
                            </tr>
                        @endforeach
                        </tbody>
                    </table>
                    <div>
                        {{ $disciplines->links('pagination::tailwind')}}
                    </div>
                </div>
            </div>
        </div>
