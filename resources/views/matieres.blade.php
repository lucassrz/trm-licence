<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between">
            ...
            <form action="{{ route('matieres') }}" method="GET">
        ...
        <div>
            <a class="p-2 bg-gray-500 hover:bg-gray-700 text-white font-bold rounded" href="{{ route('matieres/form') }}">{{ __('other.add_matiere') }}</a>
        </div>
    </x-slot>
                    ...
                    <table class="min-w-full mb-3">
                        ...
                        @foreach ($matieres as $matiere)
                            <tr class="bg-gray-100 border-b">
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{$matiere->id}}</td>
                                <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">{{$matiere->libelle}}</td>
                            </tr>
                        @endforeach
                        </tbody>
                    </table>
                    <div>
                        {{ $matieres->links('pagination::tailwind')}}
                    </div>
                </div>
            </div>
        </div>
