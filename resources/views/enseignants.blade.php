    ...
    <x-slot name="header">
        <div class="flex justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('other.enseignants') }}
            </h2>
            <form action="{{ route('enseignants') }}" method="GET">
        ...
        <div>
            <a class="p-2 bg-gray-500 hover:bg-gray-700 text-white font-bold rounded" href="{{ route('enseignants/form') }}">{{ __('other.add_enseignant') }}</a>
        </div>
    </x-slot>
                ...
                <div class="overflow-hidden">
                    <table class="min-w-full mb-3">
                        ...
                        <tr>
                            ...
                            <th scope="col" class="text-sm font-medium text-gray-900 px-6 py-4 text-left">
                                @sortablelink('nom', "Nom")
                            </th>
                            <th scope="col" class="text-sm font-medium text-gray-900 px-6 py-4 text-left">
                                @sortablelink('prenom', "Prénom")
                            </th>
                            <th scope="col" class="text-sm font-medium text-gray-900 px-6 py-4 text-left">
                        ...
                        @foreach ($enseignants as $enseignant)
                            <tr class="bg-gray-100 border-b">
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{$enseignant->id}}</td>
                                <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">{{$enseignant->nom}}</td>
                                <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">{{$enseignant->prenom}}</td>
                                <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">{{$enseignant->created_at}}</td>
                            </tr>
                        @endforeach
                        </tbody>
                    </table>
                    <div>
                        {{ $enseignants->links('pagination::tailwind')}}
                    </div>
                </div>
            </div>
        </div>
